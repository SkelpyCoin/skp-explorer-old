...
module.exports = function (app) {
    ...
    this.getBlocks = function (error, success) {
        ...
        async.doUntil(
            function (next) {
                request.get({
                    url: app.get('skp address') + blocks.url(offset, limit),
                    json : true
    ...
    this.getLastBlock = function (error, success) {
        ...
        request.get({
            url: app.get('skp address') + blocks.url(0, 1),
            json : true
        }, function (err, resp, body) {
            if (err || resp.statusCode !== 200) {
    ...
    function Peers (locator) {
        ...
        this.process = function (p, next) {
            ...
            // Check for duplicates because SKP API ignore limit/offset
    ...
    this.getPeers = function (error, success) {
        ...
        async.doUntil(
            function (next) {
                request.get({
                    url: app.get('skp address') + peers.url(offset, limit),
                    json : true
                }, function (err, resp, body) {
                    if (err || resp.statusCode !== 200) {
                        return next(err || 'Response was unsuccessful');
